...
public class CircuitManagerImpl implements CircuitManager {
        ...
        try (final PreparedStatement statement = conn.prepareStatement(retrieve);
            ...
            while (result.next()) {
                circuits.add(new Circuit(result.getString("nomeCircuito"), result.getString("stato"),
                        result.getString("localita"), result.getInt("curveDestra"), result.getInt("curveSinistra"),
                        result.getInt("lunghezza"), result.getInt("rettilineoMax"), result.getString("foto"),
                        result.getString("record"), result.getInt("pilotaRecord"), result.getInt("annoRecord")));
            }
    ...
    @Override
    public void addCircuit(String name, String state, String location, int rightHanders, int leftHanders, int lenght,
