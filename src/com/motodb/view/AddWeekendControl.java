...
public class AddWeekendControl extends ScreenControl {
    ...
    public void initialize() {
        ...
        endDateColumn.setCellValueFactory(
                cellData -> new ReadOnlyStringWrapper(cellData.getValue().getFinishDate().toString()));
    ...
    private void add() {
        try {
            weekendManager.addWeekend(Integer.parseInt(yearBox.getSelectionModel().getSelectedItem()),
                    circuitBox.getSelectionModel().getSelectedItem().getName().toString(),
                    java.sql.Date.valueOf(startDate.getValue().toString()),
                    java.sql.Date.valueOf(endDate.getValue().toString()));

            weekendTable.setItems(weekendManager.getWeekends()); // Update table
                                                                 // view
            this.clear();
        } catch (Exception e) {
            e.printStackTrace();
            ...
            public DateCell call(final DatePicker datePicker) {
                return new DateCell() {
                    ...
                    public void updateItem(LocalDate item, boolean empty) {
                        ...
                        try {
                            if (item.isBefore(new SimpleDateFormat("yyyy-MM-dd").parse(year + "-01-01").toInstant()
                                    .atZone(ZoneId.systemDefault()).toLocalDate())
                                    || item.isAfter(new SimpleDateFormat("yyyy-MM-dd").parse(year + "-12-31")
