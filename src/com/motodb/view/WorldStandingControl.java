...
public class WorldStandingControl extends ScreenControl {
    ...
    @FXML
    private TableColumn<Ranking, String> nameColumn, surnameColumn, nationColumn, numberColumn, teamColumn,
            manufacturerColumn, pointsColumn;
    ...
    private void filter() {
        ...
        classesButtons.selectedToggleProperty().addListener(new ChangeListener<Toggle>() {
            public void changed(ObservableValue<? extends Toggle> observable, Toggle oldValue, Toggle newValue) {
                ...
                if (!classesButtons.getToggles().isEmpty()) {
                    if (!rankingManager.getRankingByYearAndClass(
                            Integer.parseInt(yearsButtons.getSelectedToggle().getUserData().toString()),
                            classesButtons.getSelectedToggle().getUserData().toString()).isEmpty()) {
                        rankingTable.setItems(rankingManager.getRankingByYearAndClass(
                                Integer.parseInt(yearsButtons.getSelectedToggle().getUserData().toString()),
                                classesButtons.getSelectedToggle().getUserData().toString()));
                    } else {
